...
const captureToast = (hostEl, {
    ...
    const host = hostEl.closest?.('.thumbnail') || hostEl;
    if (!host) {
        showGlobalToast({ text, icon, duration });
        return;
    }

    let box = host.querySelector('.message_container');
...
export const bindCaptureToast = ({
 ...
 getText = (_btn, next, success) =>
     success === false ? '요청에 실패했습니다.'
         : (next ? '스크랩 되었습니다.' : '취소되었습니다.'),

 getIcon = (_btn, next, success) => next
 ...
 shouldShow = () => true,
    ...
    const key = `${_getRootId(delegateRoot)}|${buttonSelector}|${bindClick?'C':'-'}|${listen?'E':'-'}`;
    ...
    if (bindClick) {
        handler = (e) => {
            ...
            if (Object.prototype.hasOwnProperty.call(btn.dataset, 'capture')) {
                cur = (btn.dataset.capture === '1' || btn.dataset.capture === 'true') ? 1 : 0;
            } else if (btn.getAttribute('aria-pressed') === 'true' || btn.classList.contains('active')) {
                cur = 1;
            } else {
    ...
    _bindings.set(key, { root: delegateRoot, handler, eventHandler, listenTarget, listenEvent });
};