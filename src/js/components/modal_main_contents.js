// public/js/components/modal_main_contents.js
//사용 안함 hoya 251021

document.addEventListener('DOMContentLoaded', function() {
    const searchForm = document.getElementById('searchForm');
    const searchBtn = document.getElementById('searchBtn');
    const tabNavigation = document.querySelector('.modal_wrapper .tab_navigation');
    const tabContents = document.querySelectorAll('.modal_wrapper .tab_content');
    ...
    // 탭 활성화 함수 (외부에서도 호출 가능)
    function activateTab(dataIndex) {
        const indexStr = String(dataIndex);

        // 모든 탭에서 active 클래스 제거
        document.querySelectorAll('.modal_wrapper .tab_navigation button').forEach(btn => {
        ...
        // 해당 탭에 active 클래스 추가
        const targetButton = document.querySelector(`.modal_wrapper .tab_navigation button[data-index="${indexStr}"]`);
        if (targetButton) {
            targetButton.classList.add('active');
        }

        // 해당 탭 컨텐츠에 active 클래스 추가
        if (tabContents[parseInt(indexStr)]) {
            tabContents[parseInt(indexStr)].classList.add('active');
        }

        // data-index 값을 hidden input에 설정
        if (dataIndexInput) {
            dataIndexInput.value = indexStr;
        }

        console.log(`탭 ${indexStr} 활성화됨`);
    }
    ...
    function submitForm() {
        ...
        fetch('/master/keywords/contents', {
            method: 'POST',
            body: formData,
            headers: headers
        })
    ...
    // 페이지 로드 시 첫 번째 탭 활성화
    const firstTab = document.querySelector('.modal_wrapper .tab_navigation button[data-index="0"]');
    ...
    window.activateModalTab = activateTab;
