...
const bombomController = (() => {
    ...
    const findSectionData = (data, targetVtype, bchIndex = 0) => {
        ...
        if (targetVtype === "bsv") {
            ...
            if (
                bchGroups[bchIndex] &&
                bchGroups[bchIndex].groups &&
                bchGroups[bchIndex].groups[0]
            ) {
                return {
                    title:
                        bchGroups[bchIndex].groups[0].group_info?.title || "",
    ...
    const renderNoData = ($container) => {
        ...
        // 더보기 버튼 숨기기
        const $btnContainer = $container.querySelector(
    ...
    const content = {
        ...
        getContainer: () =>
            document.querySelector(".list_contents.list_newest"),
