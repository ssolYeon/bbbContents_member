...
const sabomContentController = (() => {
    ...
    // 설정 정보 객체 - dewbian
    const config = {
        API_BASE: '/api/sbList', // 상품 API 엔드포인트로 변경 - dewbian
        CARD_TYPES: {
            SHOP: 'shop_174_174',
    ...
    const typeChange = () => {
        ...
        const $btnCardType = document.querySelector('.sort_container button[data-type="array_card"]');
        const $btnListType = document.querySelector('.sort_container button[data-type="array_list"]');

        $btnCardType.addEventListener('click', (e) => {
            $container.classList.remove('type_list');
            $btnListType.classList.remove('active');
            $btnCardType.classList.add('active');
        })

        $btnListType.addEventListener('click', (e) => {
            $container.classList.add('type_list');
            $btnCardType.classList.remove('active');
            $btnListType.classList.add('active');
        })
    }
    // 정리 함수들 - dewbian
    const cleanup = {
    ...
    const initialize = async () => {
        ...
        if (!state.isGroupMode) {
            await initCategoryToggleNav({dataUrl: "/api/sbCategory"});
        }
...
document.addEventListener('DOMContentLoaded', sabomContentController.init);
