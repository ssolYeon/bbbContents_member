    ...
    const groupHaebomContents = async () => {
        const methods = {
            ...
            renderHorizontalList: ($container, group) => {
                ...
                const contents = items.map(item => cardTemplates.haebomCard(item, 'shop_164_123')).join('');
            ...
            renderSmallList: ($container, group) => {
                ...
                const contents = items.map(item => cardTemplates.haebomCard(item, 'card_174_174')).join('');
...
// API 관련 함수들 - dewbian
    ...
    const content = {
        ...
        render: (items = []) => {
            ...
            const html = items.map(item => cardTemplates.haebomCard(item, 'card_174_174')).join('');
