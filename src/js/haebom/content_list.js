...
const haebomListController = (() => {
    ...
    const config = {
        API_BASE: '/api/hbList', // 상품 API 엔드포인트로 변경
        CARD_TYPES: {
            SHOP: 'shop_174_174',
            CONTENT: 'card_174_174' // heabomController와 같은 카드 클래스로 변경
        }
    };
    ...
    const utils = {
        ...
        // 할인율 계산 함수 추가 (gabomContentController 방식 적용)
    ...
    const initialize = async () => {
        ...
        if (!state.isGroupMode) {
            renderCategoryNavigation('/api/hbCategory'); // 상품 카테고리 API로 변경
        }
        ...
        if (!state.isGroupMode) {
           await initCategoryToggleNav({dataUrl: "/api/hbCategory"}); // 상품 카테고리 API로 변경
        }
...
document.addEventListener('DOMContentLoaded', haebomListController.init);
