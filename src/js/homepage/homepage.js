                      ...
                      <div class="swiper-slide slider" role="group" aria-label="${title}">
                        <a href="${detail_url}">
                          ...
                          <div class="slider_text_container">
                            <div class="category">
								${stitle ? `<span class="series">${stitle}</span>` : ``}
								<span>${category}</span>
							</div>
                ...
                const groupsHtml = groups
                    .map((group) => {
                        ...
                        const itemsHtml = posts
                            .map((item) =>
                                cardTemplates.contentCard_home(item, "half")
                            )
                            .join("");
                        return `
    ...
    const latestSeries = async () => {
        ...
        const methods = {
            ...
            renderContent: (data = []) => {
                ...
                // 첫 번째 그룹 타이틀 처리
                const firstGroupTitle = firstGroup.group_info.title
                    .replace(/\r\n/g, "<br>")
                    .replace(/\n/g, "<br>");

                // 첫 번째 그룹 세로 리스트
                const verticalItems = firstGroup.items
                ...
                if (secondGroup) {
                    secondGroupTitle = secondGroup.group_info.title
                        .replace(/\r\n/g, "<br>")
                        .replace(/\n/g, "<br>");
                } else {
                }
                ...
                // 나머지 그룹들 가로 리스트
                const horizontalItems = restGroups
                    .flatMap((group) => {
                        return group.items
                            .map((item) =>
                                cardTemplates.seriesCard_home(item, "full")
                            )
                            .join("");
                    })
                    .join("");
            ...
            renderContent: (data = []) => {
                ...
                const groupsHtml = groups
                    .map((group) => {
                        ...
                        const itemsHtml = posts
                            .map((item) =>
                                cardTemplates.contentCard(item, "card_340_340")
                            )
                            .join("");
        ...
        const methods = {
            ...
            renderContent: (data = {}) => {
                ...
                groups.forEach((group, index) => {
                    ...
                    const itemsHtml = posts
                        .map((item) => cardTemplates.gabomCard(item, "full"))
                        .join("");
                    // 그룹 타이틀 처리 (줄바꿈 적용)
                    const formattedGroupTitle = groupTitle
                        .replace(/\r\n/g, "<br>")
                        .replace(/\n/g, "<br>");
                ...
                if ($parentContainer) {
                    // 기존 가봄 관련 섹션들 제거
                    const existingGabomSections =
                        $parentContainer.querySelectorAll(
                            ".homepage_gabom_container, .homepage_gabom_hot_container"
                        );
                    existingGabomSections.forEach((section) =>
                        section.remove()
                    );

                    // sabom_container 찾기 (이 앞에 삽입해야 함)
                    const $sabomContainer = $parentContainer.querySelector(
                        ".homepage_sabom_container"
                    );
