...
const homepageController = (() => {
            ...
            renderContent: (data = []) => {
                ...
                const groupsHtml = groups.map(group => {
                    ...
                    return `
                            ...
                            <ul role="list" class="bombom_list">
                                ${itemsHtml}
                            </ul>
                    `;
                }).join('');
    ...
    const latestSeries = async () => {
        ...
        const methods = {
            ...
            renderContent: (data = []) => {
                ...
                <ul role="list" class="series_list">
                    ${verticalItems}
                </ul>
                ${horizontalItems ? `
                    <div class="title_container">
        ...
        const methods = {
            ...
            renderContent: (data = {}) => {
                ...
                const groupsHtml = groups.map(group => {
                    ...
                    <ul role="list" class="haebom_list">
                        ${itemsHtml}
                    </ul>
                `;
                }).join('');
    ...
    const latestGabomHotContents = () =>{
        const DATA_URL = '/data/homepage/homepage_gabom_hot.json';
        ...
        const methods = {
            ...
            renderContent :  (items = []) => {
                ...
                const contents__ = items.map((item) => `
        ...
        const methods = {
            requestAPI: async () => {
                try {
                    ...
                    if (!response.ok) throw new Error("데이터 불러오기 실패");
                    const data = await response.json();
                    const groups = data?.M0600?.groups || [];
                    if (groups.length > 0) {
