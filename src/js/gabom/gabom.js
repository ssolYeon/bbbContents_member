...
// 가로 리스트 렌더링
            renderHorizontalList: ($container, group) => {
                ...
                const contents = items.map(item => cardTemplates.gabomCard(item, 'full')).join('');
...
// 더블 로우 리스트 렌더링
            renderDoubleRowList: ($container, group) => {
                ...
                const topRowRender = topRows.map(item => cardTemplates.gabomCard_02(item, 'half')).join('');
                const bottomRowRender = bottomRows.map(item => cardTemplates.gabomCard_02(item, 'half')).join('');
