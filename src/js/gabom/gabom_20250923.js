...
const gabomSubMainController = (()=>{
    ...
    const renderCarousel = ()=>{
        const DATA_URL = '/api/banners?type=gbm';
    ...
    const renderBanner = () => {
        const DATA_URL = '/api/banners?type=gbb';
    ...
    const groupGabomContents = async () => {
        const methods = {
            requestAPI: async () => {
                try {
                    // API 호출
                    const response = await fetch('/api/bbG0100', {
                        method: 'GET',
                        headers: {
            ...
            renderContent: (data = {}) => {
                ...
                //     methods.renderSubVisual($subVisual, validGroups[0]);
                // }
                ...
                validGroups.slice(0).forEach((group, index) => {
                    ...
                    if ($container) {
                        ...
                        if (viewMoreBtn) {
                            viewMoreBtn.href = '/gbl?group='+group.group_info.gidx;
                        }
...
// 가로 리스트 렌더링
            renderHorizontalList: ($container, group) => {
                ...
                const contents = items.map(item => cardTemplates.gabomCard(item, 'shop_340_221')).join('');
...
// 더블 로우 리스트 렌더링
            renderDoubleRowList: ($container, group) => {
                ...
                const topRowRender = topRows.map(item => cardTemplates.gabomCard_02(item, 'shop_164_123')).join('');
                const bottomRowRender = bottomRows.map(item => cardTemplates.gabomCard_02(item, 'shop_164_123')).join('');

                $container.innerHTML = `
                    <div class="row">
                        <ul class="row_scroll_box">${topRowRender}</ul>
                    </div>
                    <div class="row">
                        <ul class="row_scroll_box">${bottomRowRender}</ul>
                    </div>
    `;
                if (state.lazy?.refresh) {
                    state.lazy.refresh($container);
            ...
            renderContentsList: ($container, group) => {
                ...
                const contents = items.map(item => cardTemplates.gabomCard(item, 'card_361_270')).join('');
...
// API 관련 함수들 - dewbian
    ...
    const content = {
        ...
        render: (items = []) => {
            ...
            const html = items.map(item => cardTemplates.gabomCard(item, 'card_361_270')).join('');
    ...
    const initialize = ()=>{
        ...
        allmenu();
        initSearchHandler();
        initCategoryToggleNav({dataUrl:"/api/gbCategory"})
...
document.addEventListener('DOMContentLoaded', gabomSubMainController.init);
