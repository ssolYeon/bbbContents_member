...
const searchController = (() => {
  ...
  const utils = {
    ...
    resetState: () => {
      ...
      document.querySelector(".myp_searchResult_container").style.height =
        "38rem";
      state.currentPage = 1;
      state.totalPages = 1;
    ...
    updateTabUI: (activeTab) => {
      ...
      if (targetTab) {
        if (activeTab == "bb" || activeTab == "bc" || activeTab == "bs") {
          ...
          subTabNavigation.style.display = "flex";
  ...
  const categoryControl = {
        ...
        .map(
          (category) =>
            `<a href="javascript:void(0)" class="${
              category.data === state.currentCategory ? "active" : ""
            }" data-category="${category.data}">${category.name}</a>`
        )
        .join("");
        ...
        .map(
          (category) =>
            `<li><a href="javascript:void(0)" class="${
              category.data === state.currentCategory ? "active" : ""
            }" data-category="${category.data}">${category.name}</a></li>`
        )
        .join("");
    ...
    bindEvents: () => {
      ...
      const $topTggleNav = document.querySelector(
      ...
      $topTggleNav.addEventListener("click", close);
    ...
    loadContent: (url, tabType, resetPage = true) => {
      ...
      fetch(url)
        ...
        .then((data) => {
          ...
          state.hasMorePages =
            pagination.has_more_pages || state.currentPage < state.totalPages;
  ...
  const contentRenderer = {
    ...
    renderContent: (data, tabType) => {
      ...
      if (!items || items.length === 0) {
        document.getElementById("bbb_empty_container").style.display = "flex";
        document.querySelector(".myp_searchResult_container").style.height =
          "38rem";
      ...
      if (tabType === "bb" || tabType === "bc" || tabType === "bs") {
        ...
        if (tabType === "bs") {
          ...
          renderedHTML = items
            .map((item) => cardTemplates.contentCard_home(item, "half"))
            .join("");
        }
      } else {
        uiControl.removeSeriesContentStructure();
        ...
        switch (tabType) {
          case "hb":
            renderedHTML = items
              .map((item) => cardTemplates.haebomCard(item, "half"))
              .join("");
            break;
          case "gb":
            renderedHTML = items
              .map((item) => cardTemplates.gabomCard_02(item, "half"))
              .join("");
            break;
          case "sb":
            renderedHTML = items
      ...
      if (state.lazy?.refresh) {
        const searchResultContainer = document.querySelector(
          ".myp_searchResult_container"
        );
        if (searchResultContainer) {
          state.lazy.refresh(searchResultContainer);
        }
      }
    },
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (
              ...
              console.log(
                `무한 스크롤 트리거: ${state.currentPage + 1}페이지 로드`
              );
              contentLoader.loadNextPage();
            }
          });
      ...
      bindCaptureToggle({
        ...
        goLogin:
          typeof goLogin !== "undefined"
            ? goLogin
            : () => console.warn("goLogin 함수가 정의되지 않았습니다"),
