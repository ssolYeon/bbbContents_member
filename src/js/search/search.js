...
const searchController = (() => {
    ...
    const utils = {
        ...
        resetState: () => {
            document.getElementById("bbb_empty_container").style.display =
                "none";
            document.querySelector(".myp_searchResult_container").style.height = "38rem";
            state.currentPage = 1;
            state.totalPages = 1;
    ...
    const urlUtils = {
        ...
        initializeFromUrl: () => {
            ...
            if (
                urlTab &&
                ["bb", "bc", "bs", "hb", "gb", "sb"].includes(urlTab)
            ) {
    ...
    const initDeleteButton = () => {
        ...
        if (!deleteBtn || !searchInput) return;

        // 초기 상태: 입력값이 없으면 버튼 숨김
